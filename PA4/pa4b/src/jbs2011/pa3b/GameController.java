 ...
 *
 */
public class GameController  extends Activity implements OnTouchListener {
	...
	private float zoom=1.01f; // this doesn't seem to work correctly with zoom != 1f
	...
	public void levelOver(){
		...
		if (gameModel.levelOver){
			synchronized(gameModel){
				//Toast.makeText(this, "Level over" , Toast.LENGTH_LONG).show();
			gameModel.resetGame();
			//startActivity(new Intent(this, WinActivity.class));
			int level= this.gameModel.getLevel()+1;
			gameModel.createLevel(level,width,height);
